...
public class CommentoBeanDao {
	...
	public CommentoBean doRetrieveByKey (String idUtente, int idPost, String idUtentePost, GregorianCalendar data, GregorianCalendar ora) throws SQLException {
		Connection con=DriverManagerConnectionPool.getConnection();
		PreparedStatement ps=(PreparedStatement) con.prepareStatement("select * from commento where idUtente=? and idPost=? and idUtentePost=? and data=? and ora=?;");
	...
	public void doSaveOrUpdate (CommentoBean commentoBean) throws SQLException {
		Connection con=DriverManagerConnectionPool.getConnection();
		PreparedStatement ps=(PreparedStatement) con.prepareStatement("select * from commento where idUtente=? and idPost=? and idUtentePost=? and data=? and time=?;");
		ps.setString(1, commentoBean.getIdUtente());
		ps.setInt(2, commentoBean.getIdPost());
		ps.setString(3, commentoBean.getIdUtentePost());
		ps.setString(4, commentoBean.getStringData());
		...
		ResultSet resultSet=ps.executeQuery();
		if (resultSet.next()) {
			ps=(PreparedStatement) con.prepareStatement("update commento set descrizione=? where idUtente=? and idPost=? and idUtentePost=? and data=? and time=?;");
