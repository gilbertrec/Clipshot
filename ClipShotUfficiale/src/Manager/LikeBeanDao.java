...
public class LikeBeanDao {
	...
	public void doSave(LikeBean likeBean) throws SQLException {
		...
		PreparedStatement ps=(PreparedStatement) con.prepareStatement("insert into like values (?, ?, ?);");
		ps.setString(1, likeBean.getIdUtente());
		ps.setString(2, likeBean.getIdPost());
	...
	public void doDelete (LikeBean likeBean) throws SQLException {
		Connection con=DriverManagerConnectionPool.getConnection();
		java.sql.PreparedStatement ps=con.prepareStatement("delete from likeP where idUtente=? and idPost=? and idUtentePost=?;");
		ps.setString(1, likeBean.getIdUtente());
		ps.setString(2, likeBean.getIdPost());
		ps.setString(3, likeBean.getIdUtentePost());
		ps.executeUpdate();
	...
	public LikeBean doRetrieveByKey (String idUtente, String idPost, String idUtentePost) throws SQLException {
		Connection con=DriverManagerConnectionPool.getConnection();
		PreparedStatement ps=(PreparedStatement) con.prepareStatement("select * from likeP where idUtente=? and idPost=? and idUtentePost=?;");
		...
		if (resultSet.next()) {
			...
			likeBean.setIdPost(resultSet.getString("idPost"));
	...
	public ArrayList<LikeBean> doRetrieveByAll() throws SQLException {
		...
		ResultSet resultSet=query.executeQuery("select * from likeP");
